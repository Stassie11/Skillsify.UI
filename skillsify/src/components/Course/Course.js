...
export default function Course(props) {
    ...
    const onFormSubmit = () => {
        return skillsifyRepository.enrollUser(parseInt(id, 10)).then(r => r.response);
    }
    ...
    return (
        <div className="container">
            <div className="flex flex-col md:flex-row flex-1 p-6 md:p-12 bg-lightgraycustom rounded-xl mt-5">
                <img className="ml-10" src={course} alt="" style={{ width: "400px", height: "400px" }} />
                <div className="flex flex-col justify-start p-6">
                    <h1 className="mb-2 text-2xl font-bold ml-28" style={{ fontSize: "36px" }}>{data.name}</h1>
                    <p className="mb-4 text-base text-neutral-600 dark:text-neutral-200 ml-28 mr-28">
                        {data.description}
                    </p>
                    <div className="flex ml-28">
                        ...
                        <p className="text-sm mt-8 -ml-32">Start instantly and learn at your own schedule.</p>
                    </div>
                    <div className="flex ml-28 mt-4">
                        ...
                        <p className="text-sm mt-8 -ml-32">No previous knowledge needed.</p>
                    </div>
                    <div className="flex ml-28 mt-4">
                        {data.courseReviews && data.courseReviews.length > 0 ? (
                            <>
                                <Rating
                                    value={data.courseReviews.reduce((sum, i) => sum + i.rating, 0) / data.courseReviews.length}
                                />
                ...
                <div>
                    {props.isUserLoggedIn && (
                        <Link to={"/enroll-course"}>
                            <button onClick={onFormSubmit} className="inline-block rounded bg-bluecustom px-6 pb-2 pt-2 text-bold font-medium text-white mr-10 mb-10">Enroll Course</button>
                        </Link>
                    )}
                    <Link to={"/courses"}>
            ...
            {data.courseReviews && <ReviewsList courseReviews={data.courseReviews} />}
        </div>
    );
}
